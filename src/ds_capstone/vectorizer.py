...
class TextVectorizer:
    ...
    def _average_pool(self, last_hidden_states: Tensor, attention_mask: Tensor) -> Tensor:
        ...
        """
        # Expand attention mask to match last_hidden_states dimensions
        input_mask_expanded = attention_mask.unsqueeze(-1).expand(last_hidden_states.size()).float()

        # Mask out padding tokens by setting their hidden states to 0
        last_hidden = last_hidden_states * input_mask_expanded
    ...
    def embed(self, texts: Union[List[str], str], is_query: bool = False) -> np.ndarray:
        ...
        # Check for empty input list or empty strings
        if not texts or all(not text.strip() for text in texts):
            raise ValueError("Input texts cannot be empty or contain only whitespace")
